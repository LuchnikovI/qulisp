...
export QLISP_LOG_LEVELS=${QLISP_LOG_LEVELS:-"DEBUG INFO WARNING ERROR"}
...
log() {
    ...
    if [[ ! " ${QLISP_LOG_LEVELS} " =~ .*\ ${severity}\ .* ]] ; then
        log ERROR "Unexpected severity '${severity}', must be one of: ${QLISP_LOG_LEVELS}"
        severity=ERROR
    fi
